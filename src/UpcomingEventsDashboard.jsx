    ...
    <div className="m-2 text-sm font-semibold text-deep-primary">Upcoming Events</div>
      ...
      <thead className="sticky top-0 bg-soft-secondary text-dark-secondary">
        ...
        <th className="px-6 py-3 w-[10%] text-left"></th>
              <th className="px-6 py-3 w-[60%] text-left">Events</th>
              <th className="px-6 py-3 w-[30%] text-left">Date</th>
          </tr>
      </thead>
      <tbody className='pt-8 text-semibold'>
        {
          eventsData.map(eachEvent => (
            <tr key={eachEvent._id} className='border-b-3 border-bg-primary text-gray-700'>
              <td  className='px-6 py-3'>{eachEvent.no}</td>
              <td  className='px-6 py-3'>{eachEvent.events}</td>
              <td  className='px-6 '>{eachEvent.date}</td>
